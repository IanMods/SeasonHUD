...
public class SeasonHUDScreen extends Screen {
  ...
  public void render(@NotNull GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
    ...
    if (Services.PLATFORM.isModLoaded("journeymap")) {
      graphics.drawCenteredString(font, JOURNEYMAP, this.width / 2,
          MENU_PADDING + (6 * (BUTTON_HEIGHT + BUTTON_PADDING)) - (font.lineHeight + BUTTON_PADDING), 16777215);
    }
  ...
  public void init() {
    ...
    MenuButton doneButton = MenuButton.builder(MenuButtons.DONE, press -> onDone())
                                      .withPos(this.width / 2 - BUTTON_WIDTH / 2,
                                          this.height - BUTTON_HEIGHT - BUTTON_PADDING).withWidth(BUTTON_WIDTH).build();

    int row = 0;
    //noinspection ConstantValue
    CycleButton<Boolean> enableModButton = CycleButton.onOffBuilder(Config.getEnableMod()).withTooltip(
                                                          object -> Tooltip.create(Component.translatable("menu.seasonhud.tooltip.enableMod")))
    ...
    //noinspection ConstantValue
    MenuButton colorButton = MenuButton.builder(MenuButtons.COLORS, press -> ColorScreen.open(this)).withTooltip(
                                           Tooltip.create(Component.translatable("menu.seasonhud.tooltip.color")))
                                       .withPos(rightButtonX, (buttonStartY + (row * yOffset))).withWidth(BUTTON_WIDTH)
                                       .build();

    row = 1;
    hudLocationButton = CycleButton.builder(Location::getLocationName).withTooltip(
                                       object -> Tooltip.create(Component.translatable("menu.seasonhud.tooltip.hudLocation")))
                                   .withValues(Location.TOP_LEFT, Location.TOP_CENTER, Location.TOP_RIGHT,
                                       Location.BOTTOM_LEFT, Location.BOTTOM_RIGHT)
    ...
    xOffsetSlider = HudOffsetSlider.builder(Component.translatable("menu.seasonhud.slider.xOffset")).withTooltip(
                                       Tooltip.create(Component.translatable("menu.seasonhud.tooltip.xOffset")))
                                   .withValueRange(0, scaledWidth - componentWidth).withInitialValue(Config.getHudX())
                                   .withDefaultValue(Config.DEFAULT_X_OFFSET)
                                   .withBounds(rightButtonX, (buttonStartY + (row * yOffset)),
                                       BUTTON_WIDTH / 2 - BasicSlider.SLIDER_PADDING, BUTTON_HEIGHT).build();

    yOffsetSlider = HudOffsetSlider.builder(Component.translatable("menu.seasonhud" + ".slider.yOffset")).withTooltip(
                                       Tooltip.create(Component.translatable("menu.seasonhud.tooltip.yOffset")))
                                   .withValueRange(0, scaledHeight - componentHeight).withInitialValue(Config.getHudY())
                                   .withDefaultValue(Config.DEFAULT_Y_OFFSET)
                                   .withBounds(rightButtonX + BUTTON_WIDTH / 2 + BasicSlider.SLIDER_PADDING,
                                       (buttonStartY + (row * yOffset)), BUTTON_WIDTH / 2 - BasicSlider.SLIDER_PADDING,
                                       BUTTON_HEIGHT).build();

    row = 2;
    CycleButton<ShowDay> showDayButton = CycleButton.builder(ShowDay::getDayDisplayName).withTooltip(
                                                        object -> Tooltip.create(Component.translatable("menu.seasonhud.tooltip.showDay")))
                                                    ...
                                                    .create(leftButtonX, (buttonStartY + (row * yOffset)), BUTTON_WIDTH,
                                                        ...
                                                        (b, show) -> Config.setShowDay(show));

    CycleButton<Boolean> showSubSeasonButton = CycleButton.onOffBuilder(Config.getShowSubSeason()).withTooltip(
                                                              object -> Tooltip.create(Component.translatable("menu.seasonhud.tooltip.showSubSeason")))
    ...
    CycleButton<Boolean> showTropicalSeasonButton = CycleButton.onOffBuilder(Config.getShowTropicalSeason())
                                                               .withTooltip(object -> Tooltip.create(
                                                                   Component.translatable(
                                                                       "menu.seasonhud.tooltip.showTropicalSeason")))
                                                               .create(leftButtonX, (buttonStartY + (row * yOffset)),
                                                                   BUTTON_WIDTH, BUTTON_HEIGHT, Component.translatable(
                                                                       "menu.seasonhud.button.showTropicalSeason"),
                                                                   (b, show) -> Config.setShowTropicalSeason(show));

    CycleButton<Boolean> needCalendarButton = CycleButton.onOffBuilder(Config.getNeedCalendar()).withTooltip(
                                                             object -> Tooltip.create(Component.translatable("menu.seasonhud.tooltip.needCalendar")))
                                                         .create(rightButtonX, (buttonStartY + (row * yOffset)),
                                                             BUTTON_WIDTH, BUTTON_HEIGHT, Component.translatable(
    ...
    if (Services.PLATFORM.isModLoaded("journeymap")) {
      ...
      CycleButton<Boolean> journeyMapMacOSButton = CycleButton.onOffBuilder(Config.getJourneyMapMacOS()).withTooltip(
                                                                  object -> Tooltip.create(Component.translatable("menu.seasonhud.tooltip.journeyMapMacOS")))
                                                              .create(rightButtonX, (buttonStartY + (row * yOffset)),
                                                                  BUTTON_WIDTH, BUTTON_HEIGHT, Component.translatable(
