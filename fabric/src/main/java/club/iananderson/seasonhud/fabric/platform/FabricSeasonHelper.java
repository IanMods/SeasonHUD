...
public class FabricSeasonHelper implements ISeasonHelper {
  ...
  public int seasonDuration(Player player) {
    ...
    if (Common.fabricSeasonsLoaded()) {
      duration = FabricSeasons.CONFIG.getSeasonLength() / dayLength;
    }
  ...
  public ItemStack calendar() {
    ItemStack calendar = null;

    if (Common.fabricSeasonsLoaded() && Common.extrasLoaded()) {
      calendar = Registry.ITEM.get(new ResourceLocation("seasons", "season_calendar")).getDefaultInstance();
    }

    if (Common.sereneSeasonsLoaded()) {
      calendar = SSItems.CALENDAR.getDefaultInstance();
    }
